...
function boot(root, doc){
  ...
  function renderBuild(){
    ...
    FAM.forEach(fam=>{
      ...
      if(!ms.length) return;
      S.openFams=S.openFams||{};
      const on=ms.filter(m=>S.picks.some(p=>p.k===m.k)).length;
      const g=el("details","fam");
      // A group holding a chosen condition opens itself, so a pick is never
      // hidden behind a closed dropdown.
      if(S.openFams[fam.k]||on) g.setAttribute("open","");
      const sm=el("summary","fam-sum");
      sm.appendChild(el("span","fam-name",fam.label));
      sm.appendChild(el("span","fam-count", on? on+" selected" : ms.length+" options"));
      g.appendChild(sm);
      g.addEventListener("toggle",()=>{ S.openFams[fam.k]=g.open; });
